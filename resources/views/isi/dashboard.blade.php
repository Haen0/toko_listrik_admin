...
@section('content')
    <header class="bg-gray-50 shadow-md p-7">
        @auth
            <h1 class="text-2xl text-center font-bold text-slate-500">Selamat datang di Dashboard, {{ Auth::user()->owner }}!</h1>
        @else
            <h1>Anda belum login</h1>
            <p>Silakan <a href="{{ route('masuk') }}">login</a> untuk melanjutkan.</p>
        @endauth
    </header>

    <div class="bungkus py-10 w-4/5 mx-auto">
      <div class="atas flex justify-between">
          <a href="{{ route('orders.filter.by-status', ['status' => 'pesanan baru']) }}" class="siap bg-slate-200 p-3 w-44 text-center rounded-md shadow-3xl">
            <p>pesanan baru</p>
            <p>{{ $jumlah_pesanan_baru }}</p>
          </a>
          <a href="{{ route('orders.filter.by-status', ['status' => 'siap diambil']) }}" class="siap bg-slate-200 p-3 w-44 text-center rounded-md shadow-3xl">
            <p>Siap Diambil</p>
            <p>{{ $jumlah_siap_diambil }}</p>
          </a>
          <a href="{{ route('orders.filter.by-status', ['status' => 'ulasan baru']) }}" class="siap bg-slate-200 p-3 w-44 text-center rounded-md shadow-3xl">
            <p>ulasan baru</p>
            <p>{{ $jumlah_ulasan_baru }}</p>
          </a>
      </div>
      <div class="tengah">
        <div class="w-full bg-white rounded-lg shadow dark:bg-gray-800 p-4 md:p-6">
            <div class="flex justify-between mb-5">
              <div class="grid gap-4 grid-cols-2">
                <div>
                  <h5 class="inline-flex items-center text-gray-500 dark:text-gray-400 leading-none font-normal mb-2">Penjualan
                    ...
                    <div data-popover id="clicks-info" role="tooltip" class="absolute z-10 invisible inline-block text-sm text-gray-500 transition-opacity duration-300 bg-white border border-gray-200 rounded-lg shadow-sm opacity-0 w-72 dark:bg-gray-800 dark:border-gray-600 dark:text-gray-400">
                        <div class="p-3 space-y-2">
                            <h3 class="font-semibold text-gray-900 dark:text-white">Penjualan growth - Incremental</h3>
                  ...
                  <p class="text-gray-900 dark:text-white text-2xl leading-none font-bold">42 pcs</p>
                </div>
                <div>
                  <h5 class="inline-flex items-center text-gray-500 dark:text-gray-400 leading-none font-normal mb-2">Pengunjung
                  ...
                  <p class="text-gray-900 dark:text-white text-2xl leading-none font-bold">312 People</p>
                </div>
              </div>
              <div>
